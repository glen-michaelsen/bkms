...
export function CrosswordGame({ puzzle, date, initialInput, initialSolvedAt, studyDirection, language }: CrosswordGameProps) {
  if (!puzzle) return <NoPuzzle />

  return <CrosswordBoard puzzle={puzzle} date={date} initialInput={initialInput} initialSolvedAt={initialSolvedAt} studyDirection={studyDirection ?? "to_slavic"} language={language ?? "sr"} />
}

function CrosswordBoard({ puzzle, date, initialInput, initialSolvedAt, studyDirection, language }: Required<CrosswordGameProps> & { puzzle: GeneratedPuzzle }) {
            ...
            <div className="flex flex-col gap-3">
              ...
              {!isEnglishLearner && (
                ...
                >
                  ...
                  Follow @cujemoseapp for {language === "hr" ? "Croatian" : "Serbian"} content
                </a>
              )}
            </div>
          </div>
