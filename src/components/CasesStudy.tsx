    ...
    <div>
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm px-6 py-5">
          ...
          {/* Key questions (inline, only if meaningful) */}
          {active.questions.length > 0 && active.questions[0].sr !== "–" && (
            <div className="mt-5 pt-5 border-t border-slate-50">
              <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3">Key questions</p>
              <ul className="space-y-1.5">
                {active.questions.map((q, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-slate-600">
                    <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-violet-400 flex-shrink-0" />
                    <span className="font-bold text-violet-700">{getQuestion(q, language)}</span>
